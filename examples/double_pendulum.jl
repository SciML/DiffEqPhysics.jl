...
function doublependulum(du, u, params, t)
    ...
    h2 = (m2*l2^2*p1^2 + (m1 + m2) * l1^2 * p2^2 -
          2*m2*l1*l2*p1*p2*cos(θ1-θ2))/(2 * l1^2 * l2^2 * (m1 + m2*sin(θ1-θ2)^2)^2)
...
function make_pretty_gif(sol)
    ...
    for i in 1:length(timepoints)
        ...
        plot([0, x1[i]], [0, y1[i]], size = (400, 300),
            xlim = (-axis_lim, axis_lim), ylim = (-axis_lim, 1),
        ...
        if i > 8 #rainbow trail
            plot!([x2[(i - 2):i]], [y2[(i - 2):i]], alpha = 0.15,
                linewidth = 2, color = :red, label = nothing)
            plot!([x2[(i - 3):(i - 2)]], [y2[(i - 3):(i - 2)]], alpha = 0.15,
                linewidth = 2, color = :orange, label = nothing)
            plot!([x2[(i - 4):(i - 3)]], [y2[(i - 4):(i - 3)]], alpha = 0.15,
                linewidth = 2, color = :yellow, label = nothing)
            plot!([x2[(i - 6):(i - 4)]], [y2[(i - 6):(i - 4)]], alpha = 0.15,
                linewidth = 2, color = :green, label = nothing)
            plot!([x2[(i - 7):(i - 6)]], [y2[(i - 7):(i - 6)]], alpha = 0.15,
                linewidth = 2, color = :blue, label = nothing)
            plot!([x2[(i - 8):(i - 7)]], [y2[(i - 8):(i - 7)]], alpha = 0.15,
                linewidth = 2, color = :purple, label = nothing)
        end
        frame(anim)
    end
    gif(anim, fps = 30)
